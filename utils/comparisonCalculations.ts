...
export function calculateScenario(input: ScenarioInput): ScenarioResult {
    ...
    for (let month = 1; month <= totalMonths; month++) {
        ...
        timeline.push({ period: month, totalInvested: Number(currentInvested.toFixed(2)), totalAccumulated: Number(currentTotal.toFixed(2)) });
    }

    const finalValue = timeline.length > 0 ? timeline[timeline.length - 1]?.totalAccumulated : initialValue;
    const totalInvested = currentInvested;
    const totalProfit = Math.max(0, finalValue - totalInvested);
    const profitPercentage = totalInvested > 0 ? (totalProfit / totalInvested) * 100 : 0;

    return { input, finalValue: Number(finalValue.toFixed(2)), totalInvested: Number(totalInvested.toFixed(2)), totalProfit: 
                Number(totalProfit.toFixed(2)), profitPercentage: Number(profitPercentage.toFixed(2)), timeline};
}